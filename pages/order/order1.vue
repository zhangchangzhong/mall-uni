...
<view class="container">
    <view class="orders">
        <navigator :url="'./orderDetail?id=' + item.id" class="order" v-for="(item, index) of orderList" :key="item.id" :data-index="index">
          ...
          <view class="goods" v-for="(iitem, iindex) of item.orderItemList" :key="iitem.id" :data-index="iindex">
			  
                <view class="img">
                    <image :src="iitem.productPic"/>
                </view>
                <view class="info">
                    <text class="name">{{iitem.productName}}</text>
                    <text class="number">共{{item.orderItemList.length}}件商品</text>
                </view>
            </view>
<!-- 			    <view class="goods" :key="1" :data-index="iindex">
			    <view class="img">
					<image src="http://macro-oss.oss-cn-shenzhen.aliyuncs.com/mall/images/20180615/5acc5248N6a5f81cd.jpg"/>
			    </view>
			    <view class="info">
            ...
            <view class="b">
                <view class="l">实付：￥{{item.totalAmount}}</view>
                <view class="r">
                    <button class="btn" :data-order-index="index" @click="payOrder" v-if="item.payType==0">去付款</button>
                </view>
            </view>
        </navigator>
